...
{	
	...
	///////////////////////////////////////
	//Multiplayer guest on a replicated body: stream WHERE we want it and let
	//the host's sim do the moving — our copy follows via body sync. No local
	//forces, or the two sims would fight.
	if(mbNetForwarded)
	{
		mpInit->mpNetworkManager->NetGrabTarget(mpPushBody, vDesired);
		return;
	}
...
{
	///////////////////////////////////////
	//Forwarded grab: the throw happens in the HOST's sim. Compute the same
	//impulse the local path would (against the REAL mass — ours is still
	//divided while holding) and send it with the release.
	if(mbNetForwarded)
	{
		if(mpPlayer->mbCanBeThrown)
		{
			...
			mpInit->mpNetworkManager->NetGrabEnd(mpPushBody, vForward * fImpulse);
			mbNetForwarded = false; //LeaveState must not send a second (zero) end
		}
		mpPlayer->ChangeState(mPrevState);
		return;
	}
...
void cPlayerState_Grab::EnterState(iPlayerState* apPrevState)
{
	...
	//Multiplayer: on a guest this forwards the grab to the host (true) and we
	//go hands-off locally; host/offline it is bookkeeping only (false).
	mbNetForwarded = mpInit->mpNetworkManager->NetGrabBegin(
		mpPushBody, mbPickAtPoint, mvRelPickPoint, mpPlayer->mfGrabMassMul);
}
...
{
	//Multiplayer release. Guest: tells the host (idempotent — no-ops after a
	//throw already sent its impulse, or after a snatch cleared the forward).
	//Host: frees the holder slot so guests may grab the body again.
	mpInit->mpNetworkManager->NetGrabEnd(mpPushBody, cVector3f(0,0,0));
	mbNetForwarded = false;
...
void cPlayerState_Move::OnUpdate(float afTimeStep)
{
	...
	if(mlMoveCount <= 0)
	{
		if(mbNetForwarded)
		{
			//The host's copy is the moving one — brake it there.
			mpInit->mpNetworkManager->NetMoveStop(mpPushBody);
		}
		else
		{
			mpPushBody->SetAngularVelocity(0);
...
void cPlayerState_Move::OnStartExamine()
{
	if(mpPlayer->mbCanBeThrown)
	{
		...
		if(mbNetForwarded)
			mpInit->mpNetworkManager->NetMoveForce(mpPushBody, vForward * 500 * fMassMul,
													mvPickPoint, mpInit->mpGame->GetStepSize());
		else
...
bool cPlayerState_Move::OnMoveForwards(float afMul, float afTimeStep)
{
	...
	if(mbNetForwarded)
		mpInit->mpNetworkManager->NetMoveForce(mpPushBody,vForce,mvPickPoint,afTimeStep);
	else
...
bool cPlayerState_Move::OnAddYaw(float afVal)
{
	if(std::abs(afVal) > kEpsilonf)
	{
		...
		if(mbNetForwarded)
			mpInit->mpNetworkManager->NetMoveForce(mpPushBody,vForce,mvPickPoint,
													mpInit->mpGame->GetStepSize());
		else
			mpPushBody->AddForceAtPosition(vForce,mvPickPoint);
		mlMoveCount = 20;
	}
...
void cPlayerState_Move::EnterState(iPlayerState* apPrevState)
{
	...
	//Multiplayer guest: forward this move/push to the host's sim.
	mbNetForwarded = mpInit->mpNetworkManager->NetMoveBegin(mpPushBody);
}
...
{
	//Multiplayer: flush the final shove and stop forwarding.
	if(mbNetForwarded)
	{
		mpInit->mpNetworkManager->NetMoveEnd(mpPushBody);
		mbNetForwarded = false;
	}
...
{
	//Forwarded: queue the shove BEFORE ChangeState — LeaveState's NetMoveEnd
	//flushes it as the final push packet. (Same tick as the original order.)
	if(mbNetForwarded && mpPlayer->mbCanBeThrown)
	{
		float fMassMul = mpPushBody->GetMass();
		if(fMassMul>40) fMassMul = 40;
		mpInit->mpNetworkManager->NetMoveForce(mpPushBody, mvForward * 233 * fMassMul,
				mpPushBody->GetLocalPosition(), mpInit->mpGame->GetStepSize());
		mpPlayer->ChangeState(mPrevState);
		return;
	}
...
bool cPlayerState_Push::OnMoveForwards(float afMul, float afTimeStep)
{
	...
	if(fSpeed < mfMaxSpeed)
	{
		...
		else
		{
			...
			//mpPushBody->AddForceAtPosition(vForce,mpPushBody->GetLocalPosition() + mvRight*-0.4f);
			if(mbNetForwarded)
				mpInit->mpNetworkManager->NetMoveForce(mpPushBody,vForce,
														mpPushBody->GetLocalPosition(),afTimeStep);
			else
...
void cPlayerState_Push::EnterState(iPlayerState* apPrevState)
{
	...
	//Multiplayer guest: forward this push to the host's sim.
	mbNetForwarded = mpInit->mpNetworkManager->NetMoveBegin(mpPushBody);
}
