...
void cInventoryItem::Drop()
{
	...
	if(pEntity)
	{
		...
		/* multiplayer loot sharing: the same item materializes for everyone
		   (no-op offline) */
		if(mpInit->mpNetworkManager)
			mpInit->mpNetworkManager->NetOnItemDropped(msName, msEntityFile,
				pCamera->GetPosition(), pCamera->GetForward() * 3.2f);
	}
}
...
void cInventory::OnMouseUp(eMButton aButton)
{
	...
	else
	{
		...
		{
			/* Co-op loot sharing: with a session live, ANY discrete item can be
			   handed over by dragging it out — 54 of the game's .ent files say
			   CanBeDropped="false" (pickaxe included) to stop a single player
			   stranding quest gear, but passing the pickaxe over IS the point
			   here, and a dropped item stays pickable so nothing is lost.
			   Offline keeps vanilla rules. Count-based items (batteries merged
