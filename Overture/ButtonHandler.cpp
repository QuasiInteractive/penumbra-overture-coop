...
void cButtonHandler::Update(float afTimeStep)
{
	...
	else if(mState == eButtonHandlerState_MainMenu)
	{
		...
		{
			/// Mouse Movement.
			/// The menu is an 800x600 virtual-space UI and input grab is off
			/// while it is open, so drive its cursor from the ABSOLUTE mouse
			/// position (cMouseSDL already maps window pixels into the same
			/// 800x600 space the widget rects live in). Accumulating relative
			/// motion scaled by sensitivity let the OS pointer leave the
			/// window before the menu cursor reached the lower widgets.
			mpInit->mpMainMenu->SetMousePos(mpInput->GetMouse()->GetAbsPosition());
		}
