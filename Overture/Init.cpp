...
bool cInit::Init(tString asCommandLine)
{
	...
	/* FullScreen now means BORDERLESS at desktop resolution (the engine
	   overrides Width/Height with the real desktop size) — the unstable
	   SDL 1.2 exclusive mode stays retired. */
	{
		/* Per-instance override from the game dir's own multiplayer.cfg (the
		   plain key=value file the network code reads, NOT a cConfigFile):
		   two local instances share the same Documents settings.cfg, so the
		...
		if (pF)
		{
			...
			{
				int v = 0;
				if (sscanf(line, "force_windowed=%d", &v) == 1)
					bForceWindowed = (v != 0);
				else if (sscanf(line, "window_width=%d", &v) == 1)
					lW = v;
	...
	Vars.AddInt("StreamBufferSize",mpConfig->GetInt("Sound", "StreamBufferSize", 16384));
	Vars.AddInt("StreamBufferCount",mpConfig->GetInt("Sound", "StreamBufferCount", 6));
	...
	{
		const cVector2f scr = mpGame->GetGraphics()->GetLowLevel()->GetScreenSize();
		mvScreenSize.x = static_cast<long>(scr.x);
		mvScreenSize.y = static_cast<long>(scr.y);
	}
	...
	mpNetworkManager = hplNew(cNetworkManager,(this));
#ifdef PENUMBRA_MULTIPLAYER
	{
		int mulPort = mpConfig->GetInt("Multiplayer", "Port", 7777);
		if (mulPort <= 0 || mulPort >= 65536)
			mulPort = 7777;
		mpNetworkManager->SetDefaultPort((uint16_t)mulPort);
	}
#endif
	...
	/* Network pump must run in EVERY updater container (PreMenu/MainMenu/
	   MapLoadText/Default) or a host stops answering ENet + discovery the
	   moment it is not in-game. */
	mpNetworkUpdater = hplNew(cNetworkUpdater, (mpNetworkManager));
	mpGame->GetUpdater()->AddGlobalUpdate(mpNetworkUpdater);
...
void cInit::Update(float afTimeStep)
{
	...
	/* Network pump moved to cNetworkUpdater (global updater state) — pumping
	   here too would tick it twice per frame while in-game. */
}
...
void cInit::Exit()
{
	...
	// NETWORK (must teardown before world reset / player delete)
#ifdef PENUMBRA_MULTIPLAYER
	if (mpNetworkManager)
		mpConfig->SetInt("Multiplayer", "Port", (int)mpNetworkManager->GetDefaultPort());
#endif
	Log(" Exit Network multiplayer\n");
	hplDelete(mpNetworkUpdater);
	mpNetworkUpdater = NULL;
	hplDelete(mpNetworkManager);
	mpNetworkManager = NULL;
