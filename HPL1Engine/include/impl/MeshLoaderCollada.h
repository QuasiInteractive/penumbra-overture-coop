...
/* TEMP INSTRUMENTATION (multiplayer ghost models): extra Log() lines through
   the collada load path — skeleton bone count, material resolution, bone-pair
   binding. Set to 0 to strip all of it. Grep GHOST_LOADER_TRACE. */
...
namespace hpl {
	...
	public:
		cColladaScene()
		{
			...
			//Assigned ONLY when the document has HPL's <extra> scene block —
			//assimp/Blender exports don't, and every keyframe time gets
			//mfStartTime subtracted, so uninitialized garbage here turned
			//identical clip files into random animation lengths (0 or 1e32)
			//depending on what the allocation happened to contain. The
			//garbage even got baked into .collcach (StartTime attribute).
			mfStartTime = 0;
			mfEndTime = 0;
			mfDeltaTime =0;
		}
