...
//			);
	...
	void cLowLevelSoundOpenAL::SetVolume(float afVolume)
	{
		...
		/* OpenAL sums active sources onto one bus; peaks often exceed 1.0 and clip.
		   mfVolume remains the logical UI level; shave ~1.5 dB at the listener for headroom,
		   complemented by ALC_SOFT_output_limiter where OpenAL Soft exposes it (OAL_Device). */
		const float masterHeadroom = 0.85f;
		OAL_Listener_SetMasterVolume(afVolume * masterHeadroom);
	}
...
*/
									...
									int alStreamingBufferSize, int alStreamingBufferCount, bool abEnableLowLevelLog, tString asDeviceName)
	{
		...
		else
		{
			...
			/* "Generic Software" is the legacy Creative-router device name.
			   OpenAL Soft (shipped beside the rebuilt exe) rejects it and the
			   game would FatalError on boot. Honor the settings DeviceName if
			   set; empty (or the config default literal "NULL") selects the
			   system default device (alcOpenDevice(NULL)). */
			if (asDeviceName.empty() || asDeviceName == "NULL" || asDeviceName == "Generic Software")
				cInitParams.msDeviceName = "";
			else
				cInitParams.msDeviceName = asDeviceName;
		}
		...
		if (!mbInitialized)
		{
			...
			/* Empty name = system default device. (The old literal "NULL"
			   string was passed to alcOpenDevice verbatim and always failed.) */
			cInitParams.msDeviceName = "";
			mbInitialized = OAL_Init(cInitParams);
		}
		if (!mbInitialized)
		{
			...
			/* Only the legacy Creative router exposed a hardware path. With
			   OpenAL Soft the device is never "Generic *", and reporting true
			   here would flip the saved UseSoundHardware setting so the next
			   boot takes the (dead) "Generic Hardware" path. */
			mbHardwareAcc = (strcmp(OAL_Info_GetDeviceName(),"Generic Hardware") == 0);
