...
#define GetAdress(sStr) if(sStr.length()>0 && sStr[0]=='#') sStr = cString::Sub(sStr,1);
		...
		cColladaAnimation &aAnim, cColladaScene *apScene)
	{
		...
		//Rotate sids that are actually animated by this track's samplers — the
		//keyframe assembly below needs to know which of the node's rotate
		//elements take animated values and which keep their scene values.
		std::set<tString> setAnimatedRotSids;
		...
		for(size_t i=0; i< aAnim.mvSamplers.size(); i++)
		{
			...
			{
				setAnimatedRotSids.insert(sTarget);
		...
		//At runtime cMeshEntity resets each bone state to its BIND local
		//transform and cNode3D::UpdateMatrix multiplies the keyframe rotation
		//onto it (bind_rot * key_rot). The old code stored the animated
		//channels' rotation directly as the key, which is only correct when
		//the bone's bind-local rotation is identity relative to those channels
		//(Maya exports: jointOrient == bind, animated rotate rest == 0).
		//Blender/assimp exports animate the node's COMPLETE local rotation, so
		//the bind rotation was applied twice — or, with no animation at all,
		//the mesh froze at bind pose.
		//Correct general form: build the node's full local rotation for the
		//key (all rotate elements in document order, animated sids taking the
		//sampled values) and store key_rot = inverse(bind_rot) * full_rot, so
		//the runtime composition reconstructs full_rot exactly. For Maya-style
		//data this reduces to the old value.
		cMatrixf mtxInvBindRot = cMatrixf::Identity;
		if(pBone)
		{
			mtxInvBindRot = cMath::MatrixInverse(pBone->GetLocalTransform().GetRotation());
		}

		//TIME UNITS — the engine speaks SECONDS from this point on. COLLADA
		//specifies key times in seconds and every stock HPL exporter document
		//declares its scene time range in the <extra> block (mfEndTime > 0),
		//so those pass through unscaled. The assimp/Blender clip pipeline
		//emits key times as FRAME NUMBERS (0,1,2,...) and no <extra> block —
		//detectable as an empty scene time range. Convert those ONCE here, at
		//the only point keys enter a cAnimationTrack, so every downstream
		//consumer (derived animation length, cAnimationState advance/wrap/
		//IsOver, mod-side speeds) uses seconds with base speed 1. The old
		//scheme kept frame units in the keys and compensated with base speed
		//30 in the mod — two half-unit regimes that kept falling out of sync.
		const float kColladaKeyframesPerSecond = 30.0f; //Mixamo/pipeline fps
		const bool bFrameNumberDoc = (apScene->mfEndTime <= apScene->mfStartTime);
		const float fTimeScale = bFrameNumberDoc ? (1.0f / kColladaKeyframesPerSecond) : 1.0f;

		//Iterate the temporary data and add to the track.
		for(size_t i=0; i < vTempData.size(); i++)
		{
			cKeyFrame *pFrame = pTrack->CreateKeyFrame((vTempData[i].mfTime - apScene->mfStartTime) * fTimeScale);

			pFrame->trans = vTempData[i].mvTrans;
			pFrame->scale = vTempData[i].mvScale;

			//Full node-local rotation at this key, composed in document order
			//exactly like LoadColladaScene composes the node transform.
			cMatrixf mtxFullRot = cMatrixf::Identity;
			tColladaTransformListIt transIt = pNode->mlstTransforms.begin();
			for(; transIt != pNode->mlstTransforms.end(); ++transIt)
			{
				cColladaTransform &NodeTrans = *transIt;
				if(NodeTrans.msType != "rotate" || NodeTrans.mvValues.size() < 4) continue;

				cVector3f vAxis(NodeTrans.mvValues[0],NodeTrans.mvValues[1],NodeTrans.mvValues[2]);

				float fAngleDeg;
				if(setAnimatedRotSids.find(NodeTrans.msSid) != setAnimatedRotSids.end())
				{
					//Animated: recover the sampled angle from the accumulated
					//euler vector (mvRot += axis * angle, axes are axis-aligned
					//in both exporter dialects).
					int lAxis = 0;
					if(fabs(vAxis.v[1]) > fabs(vAxis.v[lAxis])) lAxis = 1;
					if(fabs(vAxis.v[2]) > fabs(vAxis.v[lAxis])) lAxis = 2;

					if(fabs(vAxis.v[lAxis]) < 0.001f) continue;
					fAngleDeg = vTempData[i].mvRot.v[lAxis] / vAxis.v[lAxis];
				}
				else
				{
					//Not animated: the node's scene value (eg Maya jointOrient).
					fAngleDeg = NodeTrans.mvValues[3];
				}

				cQuaternion qElem;
				qElem.FromAngleAxis(cMath::ToRad(fAngleDeg),vAxis);
				mtxFullRot = cMath::MatrixMul(mtxFullRot, cMath::MatrixQuaternion(qElem));
			}

			cMatrixf mtxRot = cMath::MatrixMul(mtxInvBindRot, mtxFullRot);
			cQuaternion qRot;
	...
	{
		//Blender/assimp COLLADA wraps the JOINT hierarchy in a plain node
		//(<node id="Armature" type="NODE">), while the Maya exports this pass
		//was written for keep their JOINTs at scene-root level. Descend
		//through non-JOINT nodes and attach any JOINT subtree below them to
		//the current parent bone instead of stopping dead (which produced an
		//empty skeleton for wrapped files). The wrapper's own transform is
		//not composed in: skin-attached bones get their bind pose overwritten
		//from the controller's inverse bind matrices right after this pass.
		if(apColladaNode->msType != "JOINT")
		{
			...
			{
				CreateSkeletonBone(*it, apParentBone);
			}
			return;
		}
