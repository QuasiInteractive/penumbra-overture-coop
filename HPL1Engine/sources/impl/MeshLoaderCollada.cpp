...
#define GetAdress(sStr) if(sStr[0]=='#') sStr = cString::Sub(sStr,1);
	...
	cMesh* cMeshLoaderCollada::LoadMesh(const tString& asFile,tMeshLoadFlag aFlags)
	{
		...
		if(bRet==false)
		{
			...
			Log(" loader-trace: '%s' FillStructures FAILED (parse error)\n",asFile.c_str());
#endif
			return NULL;
		}
...
#if GHOST_LOADER_TRACE
			Log(" loader-trace: '%s' skeleton created: %d bones from %d controller(s), %d joint(s) in controller 0\n",
				asFile.c_str(), pSkeleton->GetBoneNum(), (int)vColladaControllers.size(),
				vColladaControllers.empty() ? 0 : (int)vColladaControllers[0].mvJoints.size());
#endif
		}
		...
		for(int i=0;i<(int)vColladaGeometries.size(); i++)
		{
			...
			//Log("Material name: '%s'\n",sMatName.c_str());
#if GHOST_LOADER_TRACE
			Log(" loader-trace: submesh '%s' triangles material '%s' -> material file '%s'\n",
				sSubMeshName.c_str(), Geom.msMaterial.c_str(), sMatName.c_str());
#endif
			iMaterial *pMaterial;
			...
			if(pMaterial==NULL)
			{
				Error("Couldn't create material '%s' for object '%s'\n",sMatName.c_str(),
																		Geom.msName.c_str());
#if GHOST_LOADER_TRACE
				Log(" loader-trace: submesh '%s' SKIPPED: CreateMaterial('%s') returned NULL — submesh keeps NULL material\n",
					sSubMeshName.c_str(), sMatName.c_str());
#endif
				continue;
			}
...
#if GHOST_LOADER_TRACE
							lTracePairs++;
							if(lBoneIdx < 0) lTraceUnresolved++;
#endif
...
#if GHOST_LOADER_TRACE
				Log(" loader-trace: submesh '%s' bound controller '%s': %d vertex-bone pairs, %d with unresolved bone name\n",
					sSubMeshName.c_str(), pCtrl->msId.c_str(), lTracePairs, lTraceUnresolved);
#endif
...
#if GHOST_LOADER_TRACE
				if(pSkeleton)
					Log(" loader-trace: submesh '%s' has a skeleton but NO controller targeted geometry '%s' — vertex weights stay empty\n",
						sSubMeshName.c_str(), Geom.msId.c_str());
#endif
		...
		if(vColladaAnimations.empty() == false)
		{
...
#if GHOST_LOADER_TRACE
					if(lBoneIdx < 0)
						Log(" loader-trace: '%s' animation track '%s' matched no skeleton bone\n",
							asFile.c_str(), pTrack->GetName());
#endif
			...
			//Files without HPL's <extra> scene block (eg assimp/Blender
			//exports) leave start/end time at 0 -> animation length 0, which
			//pins the looping play time at 0 forever (and used to NaN-crash
			//before the cMath::Wrap guard). Derive the length from the
			//keyframes instead. THIS is the path the ghost clips actually
			//take: cAnimationManager::CreateAnimation loads the clip file as
			//a mesh and steals its embedded animation — it never calls
			//LoadAnimation, which already has this same fix.
			if(pAnimation->GetLength() <= 0)
			{
				float fMaxTime = 0;
				for(int i=0; i<pAnimation->GetTrackNum(); i++)
				{
					...
					{
						float fTime = pTrack->GetKeyFrame(pTrack->GetKeyFrameNum()-1)->time;
						if(fTime > fMaxTime) fMaxTime = fTime;
					}
				}
				//Insane values mean the keyframe times themselves are broken
				//(historically: uninitialized cColladaScene::mfStartTime
				//subtracted into every key) — a 1e32 "length" makes IsOver()
				//never fire and pins looping clips on one frame.
				if(!(fMaxTime >= 0.001f && fMaxTime <= 10000.0f))
				{
					Warning("Animation in '%s' derived an invalid length %f from keyframes — clamping (keyframe times are broken; reconvert or delete collcach)\n",
						asFile.c_str(), fMaxTime);
					fMaxTime = cMath::Clamp(fMaxTime, 0.001f, 10000.0f);
				}
				pAnimation->SetLength(fMaxTime);
#if GHOST_LOADER_TRACE
				Log(" loader-trace: '%s' embedded animation: key times converted from frame numbers @30fps, length derived from keyframes: %.3f s\n",
					asFile.c_str(), fMaxTime);
#endif
			}
		}
	...
	cAnimation* cMeshLoaderCollada::LoadAnimation(const tString& asFile)
	{
...
#if GHOST_LOADER_TRACE
			Log(" loader-trace: anim '%s' skeleton created: %d bones from %d controller(s)\n",
				asFile.c_str(), pSkeleton->GetBoneNum(), (int)vColladaControllers.size());
#endif
		}
		...
		if(vColladaAnimations.empty() == false)
		{
			...
			//Files without HPL's <extra> scene block (eg assimp/Blender
			//exports) leave start/end time at 0 -> animation length 0. A
			//looping state over a zero-length animation NaNs the time wrap
			//(cMath::Wrap divides by the span) and the NaN then crashes the
			//keyframe lookup. Derive the length from the keyframes instead.
			if(pAnimation->GetLength() <= 0)
			{
				float fMaxTime = 0;
				for(int i=0; i<pAnimation->GetTrackNum(); i++)
				...
				//Same guard as the embedded-animation path: an insane derived
				//length means broken keyframe times, not a long animation.
				if(!(fMaxTime >= 0.001f && fMaxTime <= 10000.0f))
				{
					Warning("Animation in '%s' derived an invalid length %f from keyframes — clamping (keyframe times are broken; reconvert or delete collcach)\n",
						asFile.c_str(), fMaxTime);
				...
				pAnimation->SetLength(fMaxTime);
#if GHOST_LOADER_TRACE
				Log(" loader-trace: anim '%s': key times converted from frame numbers @30fps, length derived from keyframes: %.3f s\n",
					asFile.c_str(), fMaxTime);
#endif
			}
		}
