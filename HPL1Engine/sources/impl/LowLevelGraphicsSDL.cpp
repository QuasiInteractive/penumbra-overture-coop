...
namespace hpl {
									...
									int alMultisampling, const tString& asWindowCaption)
	{
...
#ifdef WIN32
		/* Both modes, before window creation: without this Windows DPI
		   virtualization scales the window (client area no longer equals the
		   configured pixel size) — the usual reason a fresh build behaves
		   differently from the 2007 exe. Any residual client/backbuffer
		   mismatch is reconciled by ApplyWin32DrawableSize after GL is up. */
		::SetProcessDPIAware();
#endif

		if(abFullscreen)
		{
			/* "Fullscreen" = BORDERLESS window at the desktop resolution, not
			   SDL 1.2's exclusive SDL_FULLSCREEN: no display-mode switch, no
			   minimize-on-alt-tab (this is a co-op build — people alt-tab to
			   their chat constantly), and GL sizing behaves exactly like the
			   proven windowed path. Whatever size the config asked for is
			   overridden by the REAL desktop size (post-DPI-aware, so these
			   are physical pixels). */
			mlFlags |= SDL_NOFRAME;
			SDL_putenv((char *)"SDL_VIDEO_WINDOW_POS=0,0");
#ifdef WIN32
			const int lDeskW = ::GetSystemMetrics(SM_CXSCREEN);
			const int lDeskH = ::GetSystemMetrics(SM_CYSCREEN);
			if (lDeskW > 0 && lDeskH > 0)
			{
				alWidth = lDeskW;
				alHeight = lDeskH;
				mvScreenSize.x = alWidth;
...
#endif
			Log(" Fullscreen requested: borderless window at desktop %d x %d\n",
				alWidth, alHeight);
		}
		...
		/* Actual drawable can differ from requested (WM scaling, nearest mode); keep engine in sync.
		   mvVirtualSize deliberately NOT touched: it is the fixed 800x600 2D
		   coordinate space the game's menus/HUD are authored in, stretched
		   over the whole viewport by the ortho projections (stock behavior).
		   Setting it to the framebuffer size made the menu render tiny in the
		   top-left at high resolutions. */
		if (mpScreen)
		{
			if (mvScreenSize.x != mpScreen->w || mvScreenSize.y != mpScreen->h)
				Log(" SDL surface size adjusted to %d x %d (requested %d x %d)\n", mpScreen->w, mpScreen->h,
					mvScreenSize.x, mvScreenSize.y);
			mvScreenSize.x = mpScreen->w;
			mvScreenSize.y = mpScreen->h;
		}
...
#if defined(WIN32)
		/* After GL is up: match mvScreenSize / glViewport to real backbuffer (fixes corner “quarter window”). */
		ApplyWin32DrawableSize();
#else
...
#if defined(WIN32)
void cLowLevelGraphicsSDL::ApplyWin32DrawableSize()
{
	if (!mDeviceContext)
		return;
	HWND hwnd = ::WindowFromDC(mDeviceContext);
	if (!hwnd)
		return;
	RECT rc;
	...
	if (gw > 0 && gh > 0)
	{
		const long long areaClient = (long long)cw * ch;
		const long long areaGL = (long long)gw * gh;
		if (areaGL != areaClient)
			Log(" Win32 drawable: GetClientRect %dx%d, GL_VIEWPORT %dx%d\n", cw, ch, gw, gh);
		if (areaGL > areaClient)
		{
			dw = gw;
			dh = gh;
	...
	mvScreenSize.x = dw;
	mvScreenSize.y = dh;
	/* mvVirtualSize stays 800x600 — see the note after SDL_SetVideoMode. */
	...
	{
#if defined(WIN32)
		if (mDeviceContext && mpScreen && (mpScreen->flags & SDL_FULLSCREEN) == 0)
			ApplyWin32DrawableSize();
#endif
		glFlush();
		SDL_GL_SwapBuffers();
