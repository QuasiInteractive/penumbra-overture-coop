...
namespace hpl {
	...
	static void LoadScene(TiXmlElement *apRootElem, cColladaScene *apColladaScene)
	{
		...
		//Caches written before mfStartTime/mfEndTime were initialized in the
		//cColladaScene constructor have GARBAGE baked into these attributes
		//(the SaveScene path wrote the uninitialized values) — a poisoned
		//.collcach then deterministically breaks every animation in the file.
		//Self-heal: anything non-finite or absurd is really 0.
		if(!(apColladaScene->mfStartTime >= -1e6f && apColladaScene->mfStartTime <= 1e6f))
		{
			Warning("Collada cache has invalid StartTime %f — using 0 (delete core/cache/*.collcach to regenerate)\n",
				apColladaScene->mfStartTime);
			apColladaScene->mfStartTime = 0;
		}
		if(!(apColladaScene->mfEndTime >= -1e6f && apColladaScene->mfEndTime <= 1e6f))
		{
			Warning("Collada cache has invalid EndTime %f — using 0\n", apColladaScene->mfEndTime);
			apColladaScene->mfEndTime = 0;
		}
		if(!(apColladaScene->mfDeltaTime >= 0.0f && apColladaScene->mfDeltaTime <= 1e6f))
			apColladaScene->mfDeltaTime = 0;
