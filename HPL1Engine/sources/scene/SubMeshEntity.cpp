...
namespace hpl {
	...
	void cSubMeshEntity::UpdateGraphics(cCamera3D *apCamera, float afFrameTime, cRenderList *apRenderList)
	{
		if(mpDynVtxBuffer)
		{
			...
			//"Bones animate, vertices don't" tracer: prove per frame that the
			//software re-skin RUNS and WRITES for ghost bodies. ~Once per
			//second at 60 fps per submesh.
			const bool bGhostTrace =
				mpMeshEntity->GetName().compare(0,10,"GhostBody_")==0;
			static int slGhostSkinTraceCount = 0;
			const bool bGhostTraceNow = bGhostTrace && ((slGhostSkinTraceCount++ % 60)==0);
#endif
			if(mpMeshEntity->mbSkeletonPhysicsSleeping && mbGraphicsUpdated)
			{
#if GHOST_LOADER_TRACE
				if(bGhostTraceNow)
					Log(" loader-trace: '%s'/'%s' skin write SKIPPED (skeleton physics sleeping)\n",
						mpMeshEntity->GetName().c_str(), GetName().c_str());
#endif
				return;
			}
			...
			for(int vtx=0; vtx < lVtxNum; vtx++)
			{
				...
				if(*pWeight==0)
				{
					//Skipping a vertex must still advance the stream pointers;
					//the original 'continue' shifted every later vertex by one
					//slot, exploding the whole mesh if any vertex was unbound.
					//(The dyn buffer is a copy, so the vertex keeps bind pose.)
					pBindPos += lVtxStride;
					pSkinPos += lVtxStride;
					pBindNormal += 3;
					pSkinNormal += 3;
			...
			if(bGhostTraceNow)
			{
				//First weighted vertex: bind position vs what the skin loop
				//just wrote. |delta| ~0 while the trace's bone max|M-I| is
				//large = matrices posed but the write not reaching the
				//buffer/GPU; |delta| moving per sample = skinning works.
				const float *pBind = mpSubMesh->GetVertexBuffer()->GetArray(eVertexFlag_Position);
				int lProbeVtx = -1;
				for(int v=0; v<lVtxNum; ++v)
					if(mpSubMesh->mpVertexWeights[v*4] != 0) { lProbeVtx = v; break; }
				if(lProbeVtx >= 0)
				{
					const float *b = &pBind[lProbeVtx*lVtxStride];
					const float *s = &pSkinPosArray[lProbeVtx*lVtxStride];
					Log(" loader-trace: '%s'/'%s' skin write OK: %d verts, probe vtx %d bind=(%.3f,%.3f,%.3f) skinned=(%.3f,%.3f,%.3f) |d|=%.3f\n",
						mpMeshEntity->GetName().c_str(), GetName().c_str(), lVtxNum, lProbeVtx,
						b[0], b[1], b[2], s[0], s[1], s[2],
						sqrtf((s[0]-b[0])*(s[0]-b[0])+(s[1]-b[1])*(s[1]-b[1])+(s[2]-b[2])*(s[2]-b[2])));
				}
				else
					Log(" loader-trace: '%s'/'%s' skin write ran but NO weighted vertices (%d verts all weight 0)\n",
						mpMeshEntity->GetName().c_str(), GetName().c_str(), lVtxNum);
			}
#endif
