...
namespace hpl {
			...
			for(size_t i=0; i< mvAnimationStates.size(); i++)
			{
				...
				if(pAnimState->IsActive())
				{
					...
					for(int i=0; i<pAnim->GetTrackNum(); i++)
					{
						...
						//A track that never bound to a bone (bad name / missing
						//joint) has node index -1; and an animation file whose
						//skeleton has more bones than the mesh's yields indices
						//past the state vector. Either way: skip, don't index.
						if(pTrack->GetNodeIndex() < 0 ||
							pTrack->GetNodeIndex() >= (int)mvBoneStates.size()) continue;
	...
	void cMeshEntity::UpdateGraphics(cCamera3D *apCamera,float afFrameTime, cRenderList *apRenderList)
	{
		...
		if(pSkeleton)
		{
			...
			//Frozen-at-bind detector for ghost bodies: if every skinning
			//matrix is ~Identity, the skeleton is rendering bind pose (no
			//animation is driving the bone states).
			if(msName.compare(0,10,"GhostBody_")==0)
			{
				static int slGhostTraceCount = 0;
				if((slGhostTraceCount++ % 120)==0)
				{
					float fMaxDev = 0;
					for(int i=0; i< pSkeleton->GetBoneNum(); i++)
					{
						const cMatrixf &m = mvBoneMatrices[i];
						for(int r=0;r<3;++r) for(int c=0;c<4;++c)
						{
							float fDev = (float)fabs(m.m[r][c] - (r==c ? 1.0f : 0.0f));
							if(fDev > fMaxDev) fMaxDev = fDev;
						}
					}
					//Time position of the first active state: constant across
					//samples = animation not ticking; advancing = ticking.
					int lActive = 0;
					float fAnimTime = -1;
					float fAnimSpeed = 0;
					const char *pActiveName = "none";
					for(size_t i=0; i< mvAnimationStates.size(); i++)
					{
						if(mvAnimationStates[i]->IsActive())
						{
							if(lActive==0)
							{
								fAnimTime = mvAnimationStates[i]->GetTimePosition();
								fAnimSpeed = mvAnimationStates[i]->GetSpeed()*mvAnimationStates[i]->GetBaseSpeed();
								pActiveName = mvAnimationStates[i]->GetName();
							}
							lActive++;
						}
					}
					Log(" loader-trace: '%s' skin max|M-I|=%.4f activeAnims=%d/%d clip='%s' t=%.2f speed=%.1f\n",
						msName.c_str(), fMaxDev, lActive, (int)mvAnimationStates.size(),
						pActiveName, fAnimTime, fAnimSpeed);
				}
			}
#endif
